...
export default function Results(props) {
  ...
  function getMovieRecommendations(query) {
    ...
    function getMovieRecommendations(query, movies, topN = 20) {
      ...
      // Get final recommendations with genre, keywords, and popularity
      const recommendations = topIndices.map((index) => ({
        movieId: movies[index].movie_id,
        title: movies[index].title,
        genres: movies[index].genres,
        keywords: movies[index].tags,
        overview: movies[index].storyline,
        popularity: movies[index].popularity, // Assuming a popularity score exists for movies
      }));
  
      // Sort recommendations by popularity (descending order)
